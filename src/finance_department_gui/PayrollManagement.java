...
//    Viwe data form the table
    ...
    try {
        // Get selected year, month, and department values
        int selectedYear = YearChooseFromDateField.getYear();
        int selectedMonth = MonthChooseFromDateField.getMonth() + 1;  // 0-indexed, so adding 1
        ...
        String query = "SELECT * FROM employee_payrolls"
                + " INNER JOIN department ON employee_payrolls.department_department_id = department.department_id"
                + " INNER JOIN employee_position ON employee_payrolls.employee_position_employee_position_id = employee_position.employee_position_id"
                + " WHERE YEAR(employee_payrolls.payroll_date) = " + selectedYear;

        if (selectedMonth != 0) {
            query += " AND MONTH(employee_payrolls.payroll_date) = " + selectedMonth;
        }
        if (!"Select Department".equals(selectedDepartment)) {
            query += " AND department.department_name = '" + selectedDepartment + "'";
        }

        ResultSet resultSet = MySql.executeSearch("SELECT * FROM `employee_payrolls`"
                    + "INNER JOIN `department` ON `employee_payrolls`.`department_department_id`= `department`.`department_id`"
                    + "INNER JOIN `employee_position` ON `employee_payrolls`.`employee_position_employee_position_id`= `employee_position`.`employee_position_id`");
...
//        LogOut Button
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        ...
        String employee = SearchEmployeeTextField.getText();


        // Validate input fields
        if ( employee.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please Enter the Employee name", "Warning", JOptionPane.WARNING_MESSAGE);
            return;
        }
        try {
            ...
            String query = "SELECT * FROM company_promotion "
                    ...
                    + "AND type_name LIKE '%" + employee + "%'";

            // Execute the query using the existing MySql.executeSearch method
            ResultSet resultSet = MySql.executeSearch(query);

            // Clear existing table rows
            DefaultTableModel model = (DefaultTableModel)PayrollManagementTable.getModel();
            model.setRowCount(0);
    ...
    private void kButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_kButton1ActionPerformed
        ...
        try {
            ...
            if (basicSalary.isEmpty()) {
                ...
                MySql.executeUpdate("INSERT INTO `employee_payrolls` (`basic`) VALUES ('" + basicSalary + "')");
